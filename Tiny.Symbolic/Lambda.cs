...
using BigInteger = Tiny.Numeric.BigInteger;
...
namespace Tiny.Symbolic
{
    ...
    public abstract class Lambda
    {
        ...
        internal static Algebraic GetAlgebraic( Stack stack )
        {
            ...
            if ( !( arg is Algebraic ) )
            {
                Globals.Proc.ProcessInstruction( arg, true );
        ...
        internal static Algebraic evalx( string rule, Algebraic x )
        {
            try
            {
                var pgm = Globals.Parser.compile( rule );

                var save = Globals.Proc.Store;

                if ( Globals.Store == null )
                {
                    Globals.Store = new Store();

                    Globals.Store.putValue( "x", new Polynomial( new SimpleVariable( "x" ) ) );
                    Globals.Store.putValue( "X", new Polynomial( new SimpleVariable( "X" ) ) );
                    Globals.Store.putValue( "a", new Polynomial( new SimpleVariable( "a" ) ) );
                    Globals.Store.putValue( "b", new Polynomial( new SimpleVariable( "b" ) ) );
                    Globals.Store.putValue( "c", new Polynomial( new SimpleVariable( "c" ) ) );
                }

                Globals.Proc.Store = Globals.Store;
                Globals.Proc.process_list( pgm, true );
                Globals.Proc.Store = save;

                var y = GetAlgebraic( Globals.Proc.Stack );
    ...
    internal class UserProgram : Lambda
    {
        ...
        {
            var save = Globals.Proc.Store;
            var old_stack = Globals.Proc.Stack;

            Globals.Proc.Store = env;
            Globals.Proc.Stack = st;

            int ret;

            try
            {
                ret = Globals.Proc.process_list( code, true );
            }
            catch ( Exception )
            {
                ret = Processor.ERROR;
    ...
    internal class LambdaFLOAT : LambdaAlgebraic
    {
        ...
        public override int Eval( Stack stack )
        {
            ...
            var a = Globals.Proc.Store.getnum( "algepsilon" );
    ...
    internal class LambdaFORMAT : Lambda
    {
        public override int Eval( Stack stack )
        {
            ...
            if ( nargs == 1 )
            {
                ...
                {
                    Globals.fmt = new NumFmtVar( 10, 5 );

                    return 0;
                }
                else if ( "$long".Equals( arg.ToString() ) )
                {
                    Globals.fmt = new NumFmtJava();
            ...
            else if ( nargs == 2 )
            {
                ...
                Globals.fmt = new NumFmtVar( bas, nsign );
        ...
        public override int Eval( Stack stack )
        {
            ...
            while ( nargs-- > 0 )
            {
                ...
                if ( arg is string )
                {
                    ...
                    Globals.Proc.Store.putValue( s, new Polynomial( new SimpleVariable( s ) ) );
                }
            }
    ...
    internal class LambdaEXPAND : Lambda
    {
        public override int Eval( Stack stack )
        {
            ...
            if ( x is List )
            {
                Globals.Proc.process_list( ( List ) x, true );

                x = Globals.Proc.Stack.Pop();
            }
    ...
    internal class LambdaANGLE : Lambda
    {
        public override int Eval( Stack stack )
        {
            ...
            object atan2 = Globals.Proc.Store.getValue( "atan2" );
    ...
    internal class LambdaSAVE : Lambda
    {
        public override int Eval( Stack stack )
        {
            ...
            try
            {
                var stream = Globals.GetFileOutputStream( ( string ) filename, true );
                ...
                for ( var i = 1; i < size; i++ )
                {
                    ...
                    {
                        var en = Globals.Proc.Store.Keys.GetEnumerator();
                        ...
                        while ( en.MoveNext() )
                        {
                            ...
                            var val = Globals.Proc.Store.getValue( ( string ) key );
                    ...
                    {
                        var val = Globals.Proc.Store.getValue( name );
    ...
    internal class LambdaLOADFILE : Lambda
    {
        ...
        {
            var stack = Globals.Proc.Stack;

            Globals.Proc.Stack = new Stack();

            try
            {
                while ( true )
                {
                    var code = Globals.Parser.compile( stream );
                    ...
                    Globals.Proc.process_list( code, true );
                }

                stream.Close();

                Globals.Proc.Stack = stack;
            }
            catch ( Exception ex )
            {
                Globals.Proc.Stack = stack;
    ...
    internal class ExpandUser : LambdaAlgebraic
    {
        internal override Algebraic SymEval( Algebraic x1 )
        {
            ...
            var lx = Globals.Proc.Store.getValue( f.Name );
    ...
    internal class ASS : Lambda
    {
        public override int Eval( Stack stack )
        {
            ...
            for ( int i = narg - 1; i >= 0; i-- )
            {
                ...
                if ( !idxq )
                {
                    Globals.Proc.Store.putValue( name, val[ i ] );
                ...
                else
                {
                    ...
                    var lhs = new Matrix( ( Algebraic ) Globals.Proc.Store.getValue( name ) );
        ...
        internal static int lambdap( Stack stack, Lambda op )
        {
            ...
            Globals.Proc.process_list( t, true );
        ...
        internal static int lambdai( Stack st, bool sign, bool pre )
        {
            ...
            if ( !pre )
            {
                p = Globals.Proc.Store.getValue( name.Substring( 1 ) );
            }
    ...
    internal class LambdaWHO : Lambda
    {
        public override int Eval( Stack stack )
        {
            Globals.Proc.println( Globals.Proc.Store.ToString() );
    ...
    internal class LambdaADDPATH : Lambda
    {
        public override int Eval( Stack stack )
        {
            ...
            while ( narg-- > 0 )
            {
                ...
                Globals.Proc.Store.addPath( ( ( string ) s ).Substring( 1 ) );
            }
