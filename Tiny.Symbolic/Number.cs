...
namespace Tiny.Symbolic
{
    public class Number : Symbol
    {
        ...
        private BigInteger[] double2rat( double x )
        {
            ...
            var a = Globals.Proc.Store.getnum( "ratepsilon" );
        ...
        private double[] cfs( double x, double tol )
        {
            ...
            tol = System.Math.Abs( x * tol );

            var aa = System.Math.Floor( x );
            ...
            while ( System.Math.Abs( x - y[ 0 ] / y[ 1 ] ) > tol )
            {
                ra = 1.0 / ( ra - aa );
                aa = System.Math.Floor( ra );
                a.Add( aa );
                cfsd( a, y );
            }
