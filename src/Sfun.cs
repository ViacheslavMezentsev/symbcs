public class Sfun
{
	public const double EPSILON_SMALL = 1.1102230246252e-16;
	public const double EPSILON_LARGE = 2.2204460492503e-16;
	private Sfun()
	{
	}
	internal static double csevl(double x, double[] coef)
	{
	...
	private static readonly double[] COT_COEF = new double[] {.240259160982956302509553617744970e+0, -.165330316015002278454746025255758e-1, -.429983919317240189356476228239895e-4, -.159283223327541046023490851122445e-6, -.619109313512934872588620579343187e-9, -.243019741507264604331702590579575e-11, -.956093675880008098427062083100000e-14, -.376353798194580580416291539706666e-16, -.148166574646746578852176794666666e-18};
	public static double cot(double x)
	{
		double ans, ainty, ainty2, prodbg, y, yrem;
		double pi2rec = 0.011619772367581343075535053490057;
		y = JMath.abs(x);
		if (y > 4.5036e+15)
		{
			return double.NaN;
	...
	{
		return 0.43429448190325182765 * JMath.log(x);
	}
	...
	private static readonly double[] SINH_COEF = new double[] {0.1730421940471796, 0.08759422192276048, 0.00107947777456713, 0.00000637484926075, 0.00000002202366404, 0.00000000004987940, 0.00000000000007973, 0.00000000000000009};
	public static double sinh(double x)
	{
		double ans;
		double y = JMath.abs(x);
		if (double.IsNaN(x))
		{
			ans = double.NaN;
		}
	...
	{
		double ans;
		double y = JMath.exp(JMath.abs(x));
		if (double.IsNaN(x))
		{
			ans = double.NaN;
		}
	...
	private static readonly double[] TANH_COEF = new double[] {-.25828756643634710, -.11836106330053497,.009869442648006398, -.000835798662344582,.000070904321198943, -.000006016424318120,.000000510524190800, -.000000043320729077,.000000003675999055, -.000000000311928496,.000000000026468828, -.000000000002246023,.000000000000190587, -.000000000000016172,.000000000000001372, -.000000000000000116,.000000000000000009};
	...
	public static double asinh(double x)
	{
		...
		{
			ans = JMath.log(y + JMath.sqrt(y * y + 1.0));
		}
		else
		{
			ans = 0.69314718055994530941723212145818 + JMath.log(y);
		}
		if (x < 0.0)
		{
			ans = -ans;
	...
	public static double acosh(double x)
	{
		...
		{
			ans = JMath.log(x + JMath.sqrt(x * x - 1.0));
		}
		else
		{
			ans = 0.69314718055994530941723212145818 + JMath.log(x);
		}
		return ans;
	}
	private static readonly double[] ATANH_COEF = new double[] {.9439510239319549230842892218633e-1,.4919843705578615947200034576668e-1,.2102593522455432763479327331752e-2,.1073554449776116584640731045276e-3,.5978267249293031478642787517872e-5,.3505062030889134845966834886200e-6,.2126374343765340350896219314431e-7,.1321694535715527192129801723055e-8,.8365875501178070364623604052959e-10,.5370503749311002163881434587772e-11,.3486659470157107922971245784290e-12,.2284549509603433015524024119722e-13,.1508407105944793044874229067558e-14,.1002418816804109126136995722837e-15,.6698674738165069539715526882986e-17,.4497954546494931083083327624533e-18};
	public static double atanh(double x)
	{
		...
		{
			ans = 0.5 * JMath.log((1.0 + x) / (1.0 - x));
		}
		else if (y == 1.0)
		{
			ans = x * double.PositiveInfinity;
		}
		else
		{
			ans = double.NaN;
	...
	public static double gamma(double x)
	{
		...
		else
		{
			...
			{
				ans = JMath.exp((y - 0.5) * JMath.log(y) - y + 0.9189385332046727 + r9lgmc(y));
				if (x < 0.0)
				{
					double sinpiy = JMath.sin(JMath.PI * y);
					if (sinpiy == 0 || JMath.round(y) == y)
					{
						ans = double.NaN;
					}
					else
					{
						ans = -JMath.PI / (y * sinpiy * ans);
					}
				}
			}
		}
	...
	public static double logGamma(double x)
	{
		...
		if (y <= 10)
		{
			ans = JMath.log(JMath.abs(gamma(x)));
		}
		else if (x > 0)
		{
			ans = 0.9189385332046727 + (x - 0.5) * JMath.log(x) - x + r9lgmc(y);
		}
		else
		{
			sinpiy = JMath.abs(JMath.sin(JMath.PI * y));
			if (sinpiy == 0 || JMath.round(y) == y)
			{
				ans = double.NaN;
			}
			...
			{
				ans = 0.22579135264472743236 + (x - 0.5) * JMath.log(y) - x - JMath.log(sinpiy) - r9lgmc(y);
			}
		}
		return ans;
	}
	private static readonly double[] R9LGMC_COEF = new double[] {.166638948045186324720572965082e0, -.138494817606756384073298605914e-4,.981082564692472942615717154749e-8, -.180912947557249419426330626672e-10,.622109804189260522712601554342e-13, -.339961500541772194430333059967e-15,.268318199848269874895753884667e-17};
	internal static double r9lgmc(double x)
	{
		double ans;
		if (x < 10.0)
		{
	...
	public static double erfc(double x)
	{
		...
		else
		{
			...
			{
				ans = JMath.exp(-ysq) / y * (0.5 + csevl((8.0 / ysq - 5.0) / 3.0,ERFC2_COEF));
				if (x < 0)
				{
					ans = 2.0 - ans;
				}
			...
			{
				ans = JMath.exp(-ysq) / y * (0.5 + csevl(8.0 / ysq - 1,ERFCC_COEF));
				if (x < 0)
				{
					ans = 2.0 - ans;
				}
