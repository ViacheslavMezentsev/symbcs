...
public class LambdaPRIMES : Lambda
{
	public override int Eval( Stack stack )
	{
		...
		if ( !(x is Symbolic) && !( ( Symbolic ) x ).IsInteger())
		{
		    throw new JasymcaException( "Expected integer argument." );
		}

        if ( ( Symbolic ) x < Symbolic.ZERO )
		{
			x = -x;
		}
		...
		if ( x is Number )
		{
		    var xl = ( long ) ( ( Symbolic ) x ).ToComplex().Re;
	...
	internal static Vector teiler(long X)
	{
	    ...
	    while ( X % 2L == 0 )
		{
		    teiler.Add( Symbolic.TWO );
			X /= 2L;
		}

        while ( X % 3L == 0 )
		{
		    teiler.Add( Symbolic.THREE );
			X /= 3L;
		}
