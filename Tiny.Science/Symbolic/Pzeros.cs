...
public class Pzeros
{
	...
	public static void aberth(double[] ar, double[] ai, bool[] err)
	{
		...
		var poly = new doublecomplex[ ar.Length - n_zero ];

		for ( var i = 0; i < poly.Length; i++ )
		{
		    poly[ i ] = pz.dc( ar[ i + n_zero ], ai[ i + n_zero ] );
        }

		var n = poly.Length - 1;
		var eps = 2.22044604925031e-16;
		var big = double.MaxValue;
		var theSmall = double.Epsilon;
		var nitmax = 100;

		var root = new doublecomplex[n];

		var radius = new double[n];
		var errs = new bool[n + 1];
		...
		var iter = new int[1];
		iter[0] = 0;
		var apoly = new double[n + 1];
		var apolyr = new double[n + 1];
