...
namespace Tiny.Science.Numeric
{
    public class BigInteger
    {
        ...
        public BigInteger( sbyte[] val ) : this()
        {
            ...
            {
                throw new NumberFormatException( "Wrong biginteger initialization" );
            }
        ...
        private static int compareTo( BigInteger x, BigInteger y )
        {
            ...
            return Math.cmp( x.words, y.words, x_len );
        }
        ...
        private static BigInteger add( BigInteger x, BigInteger y, int k )
        {
            ...
            long carry = Math.add_n( result.words, x.words, y.words, i );
        ...
        private static BigInteger times( BigInteger x, int y )
        {
            ...
            result.words[ xlen ] = Math.mul_1( result.words, xwords, xlen, y );
            result.ival = xlen + 1;
        ...
        private static BigInteger times( BigInteger x, BigInteger y )
        {
            ...
            Math.mul( result.words, xwords, xlen, ywords, ylen );
        ...
        private static void divide( BigInteger x, BigInteger y, BigInteger quotient, BigInteger remainder, int rounding_mode )
        {
            ...
            int qlen, rlen;
            int cmpval = Math.cmp( xwords, xlen, ywords, ylen );
            ...
            else if ( ylen == 1 )
            {
                ...
                rlen = 1;
                ywords[ 0 ] = Math.divmod_1( xwords, xwords, xlen, ywords[ 0 ] );
            }
            else
            {
                int nshift = Math.count_leading_zeros( ywords[ ylen - 1 ] );

                if ( nshift != 0 )
                {
                    Math.lshift( ywords, 0, ywords, ylen, nshift );

                    int x_high = Math.lshift( xwords, 0, xwords, xlen, nshift );
                ...
                Math.divide( xwords, xlen, ywords, ylen );

                rlen = ylen;

                Math.rshift0( ywords, xwords, 0, rlen, nshift );
        ...
        public virtual BigInteger pow( int exponent )
        {
            ...
            for (; ; )
            {
                if ( ( exponent & 1 ) != 0 )
                {
                    Math.mul( work, pow2, plen, rwords, rlen );
                ...
                Math.mul( work, pow2, plen, pow2, plen );
        ...
        public virtual BigInteger gcd( BigInteger y )
        {
            ...
            len = Math.gcd( xwords, ywords, len );
        ...
        private void setShiftLeft( BigInteger x, int count )
        {
            ...
            else
            {
                ...
                int shift_out = Math.lshift( words, word_count, xwords, xlen, count );
        ...
        private void setShiftRight( BigInteger x, int count )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Math.rshift0( words, x.words, word_count, d_len, count );
        ...
        private void format( int radix, StringBuilder buffer )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (; ; )
                    {
                        int digit = Math.divmod_1( work, work, len, radix );
        ...
        public virtual string ToString( int radix )
        {
            ...
            int buf_size = ival * ( Math.chars_per_word( radix ) + 1 );
        ...
        {
            int chars_per_word = Math.chars_per_word( radix );

            int[] words = new int[ byte_len / chars_per_word + 1 ];

            int size = Math.set_str( words, digits, byte_len, radix );
        ...
        private double roundToDouble( int exp, bool neg, bool remainder )
        {
            ...
            if ( excess_bits > 0 )
            {
                m = ( ( words == null ) ? ival >> excess_bits : Math.rshift_long( words, ival, excess_bits ) );
            }
            else
            {
        ...
        public virtual int bitLength()
        {
            if ( words == null )
            {
                return Math.intLength( ival );
            }

            return Math.intLength( words, ival );
        }
        ...
        public virtual int LowestSetBit
        {
            get
            {
                ...
                if ( words == null )
                {
                    return Math.findLowestBit( ival );
                }
                else
                {
                    return Math.findLowestBit( words );
                }
            }
        }
